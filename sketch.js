...
//buttons for leaderbord and play
let leaderboard, play, backToHome, nameInput, submitName;
...
function setup() {
  ...
  leaderboard.position(100, 270);
  leaderboard.size(100, 30);
  leaderboard.mousePressed(leaderboardButtonPressed);

  play = createButton("Play");
  play.position(300, 270);
  play.size(100, 30);
  play.mousePressed(playButton);

  backToHome = createButton("back");
  backToHome.position(20, 20);
  backToHome.mousePressed(homeButton);
  backToHome.hide();
  ...
  submitName.position(220, 260);
  submitName.hide();
  submitName.mousePressed(submit);
}
...
function draw() {
  if (screen == "game") {
    ...
    push();
    if (keyIsPressed == true && key == "a" && sailorHealth > 0) {
      ...
      rotate(345);
    } else if (keyIsPressed == true && key == "d" && sailorHealth > 0) {
      //locks the ship to the screen
      if (sailorX > 500) {
        sailorX = 500;
      }
  ...
  //this is to show the start screen of the game (shows instructions). game starts when user clicks "s" key
    ...
    for (let lineY = 130; lineY <= 430; lineY += 30) {
      stroke(0);
      line(150, lineY, 350, lineY);
    }
    line(150, 130, 150, 430);
    line(350, 130, 350, 430);
    line(170, 130, 170, 430);
    line(300, 130, 300, 430);

    for (let i = 1; i <=9; i++) {
      textSize(20);
      text(i, 155, (i * 30) + 150);
    }
    topScores = getItem("topScores");
    topScores.sort((a, b) => b[1] - a[1]);
    while (topScores.length > 9) {
      topScores.pop();
    }
    for (let i = 0; i < topScores.length; i ++) {
      text(topScores[i][1], 305, (i*30) + 180);
      text(topScores[i][0], 180, (i*30) + 180);
    }
    text("#",155, 150);
    text("User Name", 175, 150)
    text("SC", 305, 150);
  }
    ...
    resetEnemyPos();
    textSize(50);
    text("Well Played", 130, 80);
    textSize(30);
    text("Name for Leaderboard ⬇", 90, 150);
    nameInput.show();
    submitName.show();
  }
  }
...
function submit() {
  if (nameInput.value() == "firstTime2409") {
    topScores.push(["test", 0]);
    topScores.push([nameInput.value(), score]);
...
function leaderboardButtonPressed() {
  screen = "leaderboard";
}
...
function mouseClicked() {
  if (bulletState == "ready" && gameTime > 20) {
    bulletX = sailorX;
    bulletY = sailorY;
    bulletState = "moving";
